		...
		addMouseListener(new MouseListener() {			
			...
			public void mouseClicked(MouseEvent e) {
				if (addPoints==true){
					...
					gridPanel.repaint();
					System.out.println("Points added: " + e.getX() + ":" + e.getY());
				}
	...
	public void generateGrid(){
		...
		for (int i=0;i<rows;i++){
			if (leftTop.x<leftBottom.x){
				leftBorder.add(new Point(leftBottom.x-i*leftWidth/rows, (int)(leftBottom.y-suma)));
			}
			else{
				leftBorder.add(new Point(leftBottom.x+i*leftWidth/rows, (int)(leftBottom.y-suma)));
			}
		...
		for (int i=0;i<rows;i++){
			if (rightTop.x<rightBottom.x){
				rightBorder.add(new Point(rightBottom.x-i*rightWidth/rows, (int)(rightBottom.y-suma)));
			}
			else{
				rightBorder.add(new Point(rightBottom.x+i*rightWidth/rows, (int)(rightBottom.y-suma)));
			}
		...
		for (int i=0;i<cols;i++){
			if (leftTop.y<rightTop.y){
				topBorder.add(new Point(leftTop.x+i*topWidth/cols, leftTop.y+i*topHeight/cols));
			}
			else{
				topBorder.add(new Point(leftTop.x+i*topWidth/cols, leftTop.y-i*topHeight/cols));
			}
		...
		for (int i=0;i<cols;i++){
			if (leftBottom.y<rightBottom.y){
				bottomBorder.add(new Point(leftBottom.x+i*bottomWidth/cols, leftBottom.y+i*bottomHeight/cols));
			}
			else{
				bottomBorder.add(new Point(leftBottom.x+i*bottomWidth/cols, leftBottom.y-i*bottomHeight/cols));
			}
...
//		for (int i=0;i<rows;i++){
//			if (leftTop.x<leftBottom.x){
//				leftBorder.add(new Point(leftTop.x+i*leftWidth/rows, leftTop.y+i*leftHeight/rows));
//			}
//			else{
//				leftBorder.add(new Point(leftTop.x-i*leftWidth/rows, leftTop.y+i*leftHeight/rows));
//			}
...
//		for (int i=0;i<rows;i++){
//			if (rightTop.x<rightBottom.x){
//				rightBorder.add(new Point(rightTop.x+i*rightWidth/rows, rightTop.y+i*rightHeight/rows));
//			}
//			else{
//				rightBorder.add(new Point(rightTop.x-i*rightWidth/rows, rightTop.y+i*rightHeight/rows));
//			}
//			System.out.println("Right border: " + rightBorder.get(i).x + ":" + rightBorder.get(i).y);
//		}
...
//	}
	...
	private void drawGrid(Graphics g){
		...
		for (int i=0;i<leftBorder.size();i++){
			g.drawLine(leftBorder.get(i).x, leftBorder.get(i).y, rightBorder.get(i).x, rightBorder.get(i).y);
		}
		for (int i=0;i<topBorder.size();i++){
			g.drawLine(topBorder.get(i).x, topBorder.get(i).y, bottomBorder.get(i).x, bottomBorder.get(i).y);
		}
	}
